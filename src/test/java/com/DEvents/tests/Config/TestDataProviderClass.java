...
public class TestDataProviderClass {
	...
	public static Object[][] GetSessions_DataProvider(){
		...
		inputData[0][8]="SessionStartDtm;FclttrName;SessionVenue;FeedbackAvg;EventId;SessionDesc;FclttrDescription;SessionId;UserStatus;SessionName;SessionEndDtm"; 		// Pass the expected data fields(delimited by ;)
		// Pass the SQL query for comparison
		inputData[0][9]="SELECT DKIOSK.KIOSK_ID as SessionId,DKIOSK.EVENT_ID as EventId,DKIOSK.KIOSK_NAME as SessionName, DKIOSK.KIOSK_DESC as SessionDesc,DKIOSK.FCLTTR_NAME as FclttrName, DKIOSK.FCLTTR_DESC as FclttrDescription, CONCAT(DKIOSK.KIOSK_STRT_DT,\"T\",DKIOSK.KIOSK_STRT_TM) AS SessionStartDtm, CONCAT(DKIOSK.KIOSK_END_DT,\"T\",DKIOSK.KIOSK_END_TM) AS SessionEndDtm,DKIOSK.KIOSK_VENUE as SessionVenue, "
				+ "(CASE WHEN (DSESN_ATTD.PRTCPNT_ATTENDANCE_ID IS NOT NULL) THEN 'JOINED' WHEN (DFED.QUESTION_ID IS NOT NULL) THEN 'FEEDBACK SUBMITTED' WHEN (DSESN.SESSION_REGSTN_ID IS NOT NULL AND DSESN_ATTD.PRTCPNT_ATTENDANCE_ID IS NULL) THEN 'REGISTERED' ELSE 'NOT REGISTERED' END) AS UserStatus,CASE WHEN DFED.QUESTION_RESPONSE IS NULL THEN 0 ELSE DFED.QUESTION_RESPONSE END AS FeedbackAvg FROM DE_EVENT_KIOSK DKIOSK INNER JOIN DE_EVENT DEVENT ON DEVENT.EVENT_ID = DKIOSK.EVENT_ID "
