...
public class TestAPI_GetEvents extends TestBase{
	...
	void getResponse() throws IOException, InterruptedException{
		...
		System.out.println("path url is :"+pathQuery);
	...
	void checkResponseTime() {
		...
		Assert.assertTrue(responseTime<7000, "Response takes more than 7 seconds");
	}
	...
	void checkResponseBody() throws ClassNotFoundException, IOException, SQLException {
		...
		dataHandler.json2xlsx(responseBody, "GetEvents");

		//Storing DB result into Excel file
		DBConnectivity.dbResultSet2xlsx(DBConnectivity.getSQLQuery(sqlQuery,sqlParms), "GetEvents", expObjects);

		Assert.assertTrue(dataHandler.compareOutputSheets("GetEvents"), "Found mismatch between Json output and Database results");
